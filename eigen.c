...
void get_evec(double Hess[3][3], double eigvec[9])
{
	...
	double *hessian, *eval_arr;
	double *max_eigvec, *mid_eigvec, *min_eigvec;
	double max_eigval = -1e20;
	...
	eval_arr = (double *) malloc(n*sizeof(double*));
	...
	for (i = 0; i < n; i++) {
        eval_arr[i] = gsl_vector_get(eval, i);
		if (eval_arr[i] < 0) eval_arr[i] *= -1;
    }

	for (i = 0; i < n; i++) {
		if (eval_arr[i] > max_eigval) {
			max_eigval = eval_arr[i];
			ind_max = i;
				for (j = 0; j < n; j++) {
	...
	for (i = 0; i < n; i++) {
		if (eval_arr[i] < min_eigval) {
			min_eigval = eval_arr[i];
			ind_min = i;
				for (j = 0; j < n; j++) {
...
void get_evec2D(double Hess[2][2], double eigvec[4])
{
	...
	int i, j, k, ind_max, ind_min, n;
	double *hessian, *eval_arr;
	double max_eigval = -1e20;
	double min_eigval = 1e20;
	double *max_eigvec, *min_eigvec;
...
void get_hessian2D(int i, int j, int k, double Hess2D[2][2])
{
	...
	double d2Jdx2_ip, d2Jdx2_im, d2Jdy2_jp, d2Jdy2_jm;
	double d2Jdxdy_jp, d2Jdxdy_jm, d2Jdydx_ip, d2Jdydx_im;
	double d2Jdx2, d2Jdxdy;
	double d2Jdydx, d2Jdy2;
	...
	Hess2D[0][0] = d2Jdx2;
	Hess2D[0][1] = d2Jdxdy - 1./(a_r[i][j][k])*dJdy[i][j][k];
	Hess2D[1][0] = d2Jdydx - 1./(a_r[i][j][k])*dJdy[i][j][k];
	Hess2D[1][1] = d2Jdy2 + a_r[i][j][k]*dJdx[i][j][k];
